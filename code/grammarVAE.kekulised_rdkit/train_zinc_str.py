...
charset = ['C', '(', ')', '1', '2', '=', 'O', 'N', '3', 'F', '[', '@', 'H', ']', '-', '#', 'S', 'l', '+', 'B', 'r', '/', '4', '\\', '5', '6', '7', 'I', 'P', '8', ' ']
...
def main():
    # 0. load dataset
    h5f = h5py.File('../../Dropbox/data/zinc_str_kekulised_rdkit_dataset.h5', 'r')
    data = h5f['data'][:]
    h5f.close()
    ...
    args = get_arguments()
    model_save = '../../Dropbox/model/zinc_vae_str_kekulised_rdkit_L' + str(args.latent_dim) + '_E' + str(args.epochs) + '_val.hdf5'
    print(model_save)
    model = MoleculeVAE()
